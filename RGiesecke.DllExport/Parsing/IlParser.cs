...
// Type: RGiesecke.DllExport.Parsing.IlParser
                ...
                for (int i = 0; i < 3; i++)
                {
                    try
                    {
                        using (Process process = new Process())
                        {
                            ...
                            ProcessStartInfo processStartInfo = new ProcessStartInfo(exePath, arguments) { UseShellExecute = false, CreateNoWindow = true, RedirectStandardOutput = true, RedirectStandardError = true };
                            ...
                            else
                            {
                                ...
                                if (flag)
                                    throw new InvalidOperationException(string.Format((IFormatProvider) CultureInfo.InvariantCulture, Resources.R_0_did_not_return_after_1_ms, (object) withoutExtension, (object) timeout));
